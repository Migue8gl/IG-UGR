...
int cont_orto = 0;
GLdouble observador_x;
GLdouble observador_y;
GLdouble observador_z;
...
void change_observer2()
{
	// posicion del observador
	glMatrixMode(GL_MODELVIEW);
	glLoadIdentity();
	glTranslatef(0, 0, -observador_z);
	glRotatef(observador_x, 1, 0, 0);
	glRotatef(observador_y, 0, 1, 0);
}
...
{
	for (int i = 0; i < helicoptero.piezas; i++)
	{
		if (color[0] == helicoptero.color_select[i].r &&
			color[1] == helicoptero.color_select[i].g &&
			color[2] == helicoptero.color_select[i].b)
		{
			if (helicoptero.activo[i] == 0)
			{
				helicoptero.activo[i] = 1;
			}
			else
			{
				helicoptero.activo[i] = 0;
			}
			glutPostRedisplay();
		}
	}
